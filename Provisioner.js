...
class Provisioner {
  ...
  async scanForWiFi() {
    try {
      ...
      if (status2 == 0) {
        if (scanFinished == 1) {
          console.log(
            `Scan status request successful! Found ${count} networks.`
          );
          let networks = [];
  ...
  async _sendSessionCmd0() {
    try {
      ...
      const sessionData_1 = proto.SessionData.deserializeBinary(
  ...
  async _sendSessionCmd1() {
    ...
    const popByteHashed = await Crypto.digest(
    ...
    let chunk1 = CryptoJS.enc.Hex.parse(
      this.bytesToHex(this.devicePublicKey.slice(0, 16))
    );
    let chunk2 = CryptoJS.enc.Hex.parse(
      this.bytesToHex(this.devicePublicKey.slice(16))
    );
    ...
    console.log(
      ...
      encryptedPublicKey.toString(CryptoJS.enc.Utf8)
    );

    s1SessionCmd1.setClientVerifyData(this.hexToBytes(encryptedPublicKey));

    console.log(
      "SessionCmd1 payload set with encrypted device public key: ",
      s1SessionCmd1.getClientVerifyData()
    );
    ...
    const sessionData_3 = proto.SessionData.deserializeBinary(
      new Uint8Array(responseData_2)
    );
  ...
  verifyDevice() {
    ...
    const dataChunck_1 = CryptoJS.enc.Hex.parse(
      this.bytesToHex(this.deviceVerify.slice(0, 16))
    );
    const dataChunck_2 = CryptoJS.enc.Hex.parse(
      this.bytesToHex(this.deviceVerify.slice(16, 32))
    );

    const decryptedBytesChunck_1 = this.encrypter
      .process(dataChunck_1)
      .toString();
    const decryptedBytesChunck_2 = this.encrypter
      .process(dataChunck_2)
      .toString();

    console.log("Decrypted Bytes Chunk 1: ", decryptedBytesChunck_1);
    console.log("Decrypted Bytes Chunk 2: ", decryptedBytesChunck_2);

    const decryptedDeviceVerify =
      decryptedBytesChunck_1 + decryptedBytesChunck_2;
